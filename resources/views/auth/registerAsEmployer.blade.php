<x-guest-layout>
    <x-auth-card>
        ...
        <form method="POST" action="{{ route('registerAsEmployer') }}">
            @csrf
            ...
            <!-- Name -->
                ...
                <!-- Company Name -->
                <div>
                    <x-label for="companyName" :value="__('Название компании')" />

                    <x-input id="companyName" class="block mt-1 w-full" type="text" name="companyName" :value="old('Название компании')" required autofocus />
                </div>
            <!-- Email Address -->
            <div class="mt-4">
                <x-label for="email" :value="__('Почта компании')" />

                <x-input id="email" class="block mt-1 w-full" type="email" name="email" :value="old('Почта компании')" required />
            </div>
